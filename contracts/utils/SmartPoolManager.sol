...
library SmartPoolManager {
    ...
    function updateWeight(IConfigurableRightsPool self, IBPool bPool, address token, uint newWeight) external {
        ...
        if (newWeight < currentWeight) {
            ...
            poolShares = BuniSafeMath.bmul(totalSupply, BuniSafeMath.bdiv(deltaWeight, totalWeight));

            // deltaBalance = currentBalance * (deltaWeight / currentWeight)
            deltaBalance = BuniSafeMath.bmul(currentBalance, BuniSafeMath.bdiv(deltaWeight, currentWeight));
            ...
            require(
                BuniSafeMath.badd(totalWeight, deltaWeight) <= BuniConstants.MAX_TOTAL_WEIGHT,
        ...
        for (uint i = 0; i < tokens.length; i++) {
            ...
            if (gradualUpdate.startWeights[i] != gradualUpdate.endWeights[i]) {
                if (gradualUpdate.endWeights[i] < gradualUpdate.startWeights[i]) {
                    ...
                    weightDelta = BuniSafeMath.bsub(gradualUpdate.startWeights[i], gradualUpdate.endWeights[i]);
                    ...
                    // First get the total weight delta
                    weightDelta = BuniSafeMath.bsub(gradualUpdate.endWeights[i], gradualUpdate.startWeights[i]);
                    // And the amount it should change per block = total change/number of blocks in the period
                    deltaPerBlock = BuniSafeMath.bdiv(weightDelta, blockPeriod);
                    //deltaPerBlock = bdivx(weightDelta, blockPeriod);

                    // newWeight = startWeight + (blocksElapsed * deltaPerBlock)
                    newWeight = BuniSafeMath.badd(
                        gradualUpdate.startWeights[i],
                        BuniSafeMath.bmul(blocksElapsed, deltaPerBlock)
                    );
                }
    ...
    function commitAddToken(
        ...
        require(
            BuniSafeMath.badd(bPool.getTotalDenormalizedWeight(), denormalizedWeight) <= BuniConstants.MAX_TOTAL_WEIGHT,
            "ERR_MAX_TOTAL_WEIGHT"
        );
    ...
    function applyAddToken(
        ...
        require(
            BuniSafeMath.bsub(block.number, newToken.commitBlock) >= addTokenTimeLockInBlocks,
        ...
        uint poolShares = BuniSafeMath.bdiv(
            BuniSafeMath.bmul(totalSupply, newToken.denorm),
    ...
    function removeToken(IConfigurableRightsPool self, IBPool bPool, address token) external {
        ...
        uint poolShares = BuniSafeMath.bdiv(
            BuniSafeMath.bmul(totalSupply, bPool.getDenormalizedWeight(token)),
            bPool.getTotalDenormalizedWeight()
        );
    ...
    function updateWeightsGradually(
        ...
        require(
            BuniSafeMath.bsub(endBlock, gradualUpdate.startBlock) >= minimumWeightChangeBlockPeriod,
    ...
    function joinPool(
        ...
        uint ratio = BuniSafeMath.bdiv(poolAmountOut, BuniSafeMath.bsub(poolTotal, 1));
        ...
        for (uint i = 0; i < tokens.length; i++) {
            ...
            uint tokenAmountIn = BuniSafeMath.bmul(ratio, BuniSafeMath.badd(bal, 1));
    ...
    function exitPool(
        ...
        uint ratio = BuniSafeMath.bdiv(pAiAfterExitFee, BuniSafeMath.badd(poolTotal, 1));
        ...
        for (uint i = 0; i < tokens.length; i++) {
            ...
            uint bal = bPool.getBalance(t);
            // Subtract 1 to ensure any rounding errors favor the pool
            uint tokenAmountOut = BuniSafeMath.bmul(ratio, BuniSafeMath.bsub(bal, 1));
    ...
    function joinswapExternAmountIn(
        ...
        require(
            tokenAmountIn <= BuniSafeMath.bmul(bPool.getBalance(tokenIn), BuniConstants.MAX_IN_RATIO),
            "ERR_MAX_IN_RATIO"
        );
    ...
    function exitswapPoolAmountIn(
        ...
        require(
            tokenAmountOut <= BuniSafeMath.bmul(bPool.getBalance(tokenOut), BuniConstants.MAX_OUT_RATIO),
            "ERR_MAX_OUT_RATIO"
        );
